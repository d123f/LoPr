...
class CrossentropyLoss_3D(torch.nn.CrossEntropyLoss):
    ...
    def forward(self, inp, target):
        ...
        return super(CrossentropyLoss_3D, self).forward(inp, target)
...
class LabelSmoothing_3D(torch.nn.Module):
    """NLL loss with label smoothing.
    """
    def __init__(self, smoothing=0.0, weight=None):
        """Constructor for the LabelSmoothing module.
        :param smoothing: label smoothing factor
        """
        super(LabelSmoothing_3D, self).__init__()
        self.confidence = 1.0 - smoothing
        self.smoothing = smoothing
        self.weight = weight
    ...
    def forward(self, inp, target):
        ce = CrossentropyLoss_3D(weight=self.weight)
        ce_loss = ce(inp,target)
