...
class DataGenerator(Dataset):
    ...
    def __init__(self, path_list, roi_number=None, num_class=2, transform=None, repeat_factor=1.0):
        ...
        self.names = [os.path.basename(path) for path in path_list]
    ...
    def __getitem__(self,index):
        ...
        try:
            ...
            print(f"Error occurred when reading file {self.path_list[index]}")
            raise e
...
def get_data_loader(data_path=[], data_type='train',repeat_factor=1.0,scale=1.0,num_classes=4,channels=1,input_shape=(512,512),batch_size=1):

        assert len(data_path) != 0

        if data_type == 'train':
            data_transformer = transforms.Compose([
                Trunc_and_Normalize(scale,channels),
                CropResize(dim=input_shape,
                ...
                # RandomAdjust2D(),
                RandomNoise2D(),
                To_Tensor(num_class=num_classes,channels=channels)
            ])
